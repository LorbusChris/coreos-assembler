...
/*
	...
	"buildv1" is the sub-command that should be used as an
	the container entrypoint, i.e.:
		/usr/bin/dumbinit /usr/bin/gangplank buildv1
...
import (
	...
	"github.com/coreos/gangplank/pkg/buildv1"
)

var (
	cmdBuildv1 = &cobra.Command{
		Use:   "buildv1",
		Short: "Execute as an OpenShift Build v1",
		Run:   runBuildv1,
	}
...
func init() {
	cmdRoot.AddCommand(cmdBuildv1)
}

// runBuilder executes the Build with a Custom Build Strategy based on
// source or binary build strategies.
func runBuildv1(c *cobra.Command, args []string) {
	defer cancel()
	defer ctx.Done()
