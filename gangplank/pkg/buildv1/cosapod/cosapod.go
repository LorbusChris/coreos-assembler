...
var (
	...
	ocpRequirements = v1.ResourceList{
		v1.ResourceCPU:    baseCPU,
		v1.ResourceMemory: baseMem,
		shared.KvmLabel:   *resource.NewQuantity(1, ""),
	}
)
...
type CosaPodder interface {
	WorkerRunner(ctx clustercontext.ClusterContext, envVar []v1.EnvVar, cosaSrvDir string) error
}
...
func NewCosaPodder(
	...
	apiBuild *buildapiv1.Build,
	index int,
	cosaSrvDir string) (CosaPodder, error) {
	...
	if ac != nil {
		...
		if err := cp.addVolumesFromSecretLabels(cosaSrvDir); err != nil {
			return nil, fmt.Errorf("failed to add secret volumes and mounts: %w", err)
		}
		if err := cp.addVolumesFromConfigMapLabels(cosaSrvDir); err != nil {
...
func (cp *cosaPod) getPodSpec(envVars []v1.EnvVar) *v1.Pod {
	...
	cosaBasePod := v1.Container{
		Name:  podName,
		Image: cp.apiBuild.Spec.Strategy.CustomStrategy.From.Name,
		Command: []string{
			"/usr/bin/dumb-init",
		},
		Args: []string{
...
`, strings.Join(cp.ocpInitCommand, "\n"))}
	...
	return &v1.Pod{
		...
		Spec: v1.PodSpec{
			...
			ServiceAccountName:            cp.apiBuild.Spec.ServiceAccount,
...
func (cp *cosaPod) WorkerRunner(ctx clustercontext.ClusterContext, envVars []v1.EnvVar, cosaSrvDir string) error {
	cluster, err := clustercontext.GetCluster(ctx)
	if err != nil {
		return err
	}
	if cluster.GetInCluster() {
		return clusterRunner(ctx, cp, envVars, cosaSrvDir)
	}
	return podmanRunner(ctx, cp, envVars, cosaSrvDir)
}

func clusterRunner(ctx clustercontext.ClusterContext, cp *cosaPod, envVars []v1.EnvVar, cosaSrvDir string) error {
		...
		case events, ok := <-w.ResultChan():
			...
			case v1.PodRunning:
				...
				for _, c := range pod.Spec.InitContainers {
					...
					if err := cp.streamPodLogs(logStarted[c.Name], pod, c.Name, cosaSrvDir); err != nil {
...
func (cp *cosaPod) streamPodLogs(logging *bool, pod *v1.Pod, container, cosaSrvDir string) error {
	cs, ns, err := clustercontext.GetClient(cp.clusterCtx)
	if err != nil {
		return err
	}
...
func podmanRunner(ctx clustercontext.ClusterContext, cp *cosaPod, envVars []v1.EnvVar, cosaSrvDir string) error {
	// Populate pod envvars
	envVars = append(envVars, v1.EnvVar{Name: shared.LocalPodEnvVar, Value: "1"})
	mapEnvVars := map[string]string{
		shared.LocalPodEnvVar: "1",
	}
	...
	s.CapAdd = podmanCaps
	s.Name = podSpec.Name
	s.Entrypoint = []string{"/usr/bin/dumb-init", "/usr/bin/gangplank", "buildv1"}
	...
	srvDir := clusterCtx.GetPodmanSrvDir()
	if srvDir == "" {
	...
	s.Command = []string{"/usr/bin/gangplank", "buildv1"}
