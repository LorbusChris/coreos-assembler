...
/*
	mountReference describes secrets or configMaps that are mounted
	as volumes. In general, these volumes contain data that is used by
	systems level tooling use as Kerberos, CA certs, etc.
*/

// mountReference is mapping of secrets or a configmap
...
func volMaps(cosaSrvDir string) map[string]mountReference {

	return map[string]mountReference{
...
func (cp *cosaPod) addVolumesFromConfigMapLabels(cosaSrvDir string) error {
	ac, ns, err := clustercontext.GetClient(cp.clusterCtx)
	if err != nil {
		return err
	}
	...
	for _, cfgMap := range cfgMaps.Items {
		if err := cp.addVolumeFromObjectLabel(cfgMap.GetObjectMeta(), cfgMap.Data, cosaSrvDir); err != nil {
			return err
		}
...
// adds the defined volume mount from volMaps.
func (cp *cosaPod) addVolumesFromSecretLabels(cosaSrvDir string) error {
	ac, ns, err := clustercontext.GetClient(cp.clusterCtx)
	if err != nil {
		return err
	}
	...
	for _, secret := range secrets.Items {
		if err := cp.addVolumeFromObjectLabel(secret.GetObjectMeta(), toStringFields(secret.Data), cosaSrvDir); err != nil {
			return err
		}
...
func (cp *cosaPod) addVolumeFromObjectLabel(obj metav1.Object, fields stringFields, cosaSrvDir string) error {
	oName := obj.GetName()
