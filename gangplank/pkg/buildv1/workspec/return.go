...
type Returner interface {
	Run(ctx context.Context, cosaSrvDir string) error
}
...
func (r *Return) Run(ctx context.Context, cosaSrvDir string) error {
	...
	b, path, err := cosa.ReadBuild(cosaSrvDir, "", util.BuilderArch())
	if err != nil {
		return err
	}
	if b == nil {
	...
	bJSONpath := filepath.Join(cosaSrvDir, "builds", shared.CosaBuildsJSON)
	...
	// Get the builds
	keyPath := filepath.Join(b.BuildID, util.BuilderArch())
	for _, f := range files {
		if f.IsDir() {
			continue
		}
