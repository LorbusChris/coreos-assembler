...
import (
	...
	gp_errs "github.com/coreos/gangplank/pkg/errors"
...
func ReadBuild(dir, buildID, arch string) (*Build, string, error) {
	...
	if buildID == "" {
		...
		if err == nil {
			...
			if !ok {
				return nil, "", gp_errs.ErrNoBuildsFound
			}
			buildID = latest
		}
	}
	...
	f, err := os.Open(filepath.Join(p, shared.CosaMetaJSON))
	if err != nil {
...
func buildParser(r io.Reader) (*Build, error) {
	...
	if errs := cosaBuild.Validate(); len(errs) > 0 {
		return nil, errors.Wrapf(gp_errs.ErrMetaFailsValidation, "%v", errs)
	}
